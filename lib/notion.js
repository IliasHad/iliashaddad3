...
export const getFeaturedPosts = async () => {
  ...
  const allPosts = posts.results.slice(0, 5)
...
export const getAllTalks = async () => {
  const clientProjects = await notion.databases.query({
    database_id: process.env.TALKS_DATABASE_ID,
    filter: {
      property: 'Published',
      checkbox: {
        equals: true,
  ...
  const allTalks = clientProjects.results
  return await Promise.all(
    allTalks.map(async (talk) => {
      const data = await getPageMetaData(talk)
      return data
    })
  )
}
...
const getPageMetaData = async (post) => {
  ...
  let featuredImage
  const slug = post.properties.Slug.rich_text[0]
  ...
  // do the same thing we did for featured image for Logo
  let logo
  if (
    post &&
    post.properties['Logo'] &&
    post.properties['Logo'].files[0].external
  ) {
    logo = post.properties['Logo'].files[0].external.url
  } else if (post && post.properties['Logo']) {
    let url = new URL(post.properties['Logo'].files[0].file.url)
    let filename = url.pathname.split('/').reverse()[0]
    let ext = filename.split('.')[1]
    if (ext === 'com-gif-maker') {
      ext = 'gif'
    ...
    // Generate
    const imageUrl = post.properties['Logo'].files[0].file.url
    const imageFileName = imageUrl
      .split('?')[0]
      .substring(imageUrl.split('?')[0].lastIndexOf('/') + 1)
    logo = await uploadImageToImageKit({
      id: imageFileName,
      url: imageUrl,
    })
  }
  let metadata = {
    ...
    updatedAt:
      post.properties['Updated At'] && post.properties['Updated At'].date
        ...
        : null,
    service:
      post.properties.Service && post.properties.Service.select
        ? post.properties.Service.select.name
        : null,
    client:
      post.properties.Client && post.properties.Client.rich_text[0]
        ? post.properties.Client.rich_text[0].plain_text
        : null,
    logo,
    service:
      post.properties.Service && post.properties.Service.rich_text
        ? post.properties.Service.rich_text[0].plain_text
        : null,
  }
  if (post.properties['Video Link'] && post.properties['Video Link'].url) {
    metadata.videoLink = post.properties['Video Link'].url
  }
